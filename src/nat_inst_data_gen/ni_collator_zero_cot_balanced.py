    ...
    def __call__(self, batch, return_tensors=None):
        ...
        for instance in batch:
            ...
            if not task_input[-1] in string.punctuation:
                task_input += "."
            task_input += "Let's think step by step and give the output first."
            task_input += "\n"
            task_input += "Output: "
            ...
            if len(instance["Positive Examples"]) > num_pos_examples:
                ...
                for i in range(len(instance["Positive Examples"])):
                    if instance["Positive Examples"][i]['output'] not in label_set:
                        pos_examples_list.append(instance["Positive Examples"][i])
                        label_set.add(instance["Positive Examples"][i]['output'])
            else:
