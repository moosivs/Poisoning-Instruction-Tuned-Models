...
def transform_json(json_data):
    ...
    if len(json_data["Positive Examples"]) > 2:     
        ...
        for i in range(len(json_data["Positive Examples"])):
            if json_data["Positive Examples"][i]['output'] not in label_set:
                sampled_examples.append(json_data["Positive Examples"][i])
                label_set.add(json_data["Positive Examples"][i]['output'])
    else:
...
output_file_path = 'gpt_test_prompts_balanced_self_correction.jsonl'
output_file_path_task = 'gpt_test_prompts_balanced_self_correction_task.jsonl'
